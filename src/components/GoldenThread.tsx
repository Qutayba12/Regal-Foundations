...
const DRAW = 1.6; // seconds for the thread to draw across
const vp = { once: true, margin: "-100px" } as const;

const icons: Record<string, React.ReactNode> = {
  // Building Today — brick wall
  trowel: (
    <path d="M3 3h18v18H3zM3 9h18M3 15h18M9 3v6M15 9v6M9 15v6M15 15v6" />
  ),
  // Supporting Tomorrow — shield
  shield: <path d="M12 3l7 3v6c0 4.5-3 7.6-7 9-4-1.4-7-4.5-7-9V6l7-3z" />,
  // Standing Forever — crown
...
export default function GoldenThread() {
  ...
  const node = (i: number) => (
    <motion.div
      ...
      animate={{
        ...
        opacity: on ? 1 : 0,
        boxShadow: on
          ? "0 0 24px 5px rgba(201,162,75,0.4)"
  ...
  const copy = (i: number, className: string) => (
    <motion.div
      className={className}
      initial={false}
    ...
    >
      <h3 className="font-display text-xl uppercase tracking-wide text-gold-gradient">
        {pillars[i].title}
      </h3>
      ...
      <div className="container-x">
        ...
        <div ref={ref} className="mt-16">
          ...
          <div className="relative hidden lg:block">
            ...
            >
              ...
              <motion.line
                ...
                stroke="url(#gtGradH)" strokeWidth="2.5" strokeLinecap="round"
                pathLength={1} filter="url(#gtGlowH)"
                initial={false}
                animate={{ pathLength: on ? 1 : 0 }}
                transition={{ duration: reduce ? 0 : DRAW, ease: [0.22, 1, 0.36, 1] }}
              />
              {on && !reduce && (
                <line
                  x1="0" y1="32" x2="1000" y2="32"
                  stroke="#FFF7E6" strokeWidth="3" strokeLinecap="round"
                  pathLength={1} strokeDasharray="0.05 0.95"
                  className="animate-threadflow" filter="url(#gtGlowH)" opacity="0.9"
                />
              )}
            </svg>
          ...
          <div className="relative lg:hidden">
            <svg
              className="absolute left-0 top-8 h-[calc(100%-4rem)] w-16 overflow-visible"
              viewBox="0 0 64 1000"
              preserveAspectRatio="none"
              fill="none"
              aria-hidden
            >
              ...
              <motion.line
                x1="32" y1="0" x2="32" y2="1000"
                stroke="url(#gtGradV)" strokeWidth="2.5" strokeLinecap="round"
                pathLength={1} filter="url(#gtGlowV)"
                initial={false}
                animate={{ pathLength: on ? 1 : 0 }}
                transition={{ duration: reduce ? 0 : DRAW, ease: [0.22, 1, 0.36, 1] }}
              />
              {on && !reduce && (
                <line
                  ...
                  stroke="#FFF7E6" strokeWidth="3" strokeLinecap="round"
                  pathLength={1} strokeDasharray="0.05 0.95"
                  className="animate-threadflow" filter="url(#gtGlowV)" opacity="0.9"
                />
              )}
            </svg>
